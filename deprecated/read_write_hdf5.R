#' @title Read or Write Cluster-Based Time-Series Datasets from HDF5
#' @name Read_Write_HDF5
#' @description
...
read_clustered_dataset <- function(file, scan_names = NULL, prefer_reduced = FALSE) {
  ...
  fh <- open_h5(file, mode = "r") # Use helper
  h5obj <- fh$h5
  ...
  # shape: (nVox, 3)
  voxel_coords <- h5obj[["voxel_coords"]][, ]
  ...
  #    If summary_only => return H5Reduced
  #    Otherwise => H5Clustered
...
write_clustered_dataset <- function(file,
  ...
  # Write header dim using h5_write
  h5_write(file, "/header/dim", hdr_dim_arr, overwrite = TRUE)
  ...
  pixdim <- c(0,
              ...
              scan_metadata[[1]]$TR,
              0, 0, 0)
  # Write header pixdim using h5_write
  h5_write(file, "/header/pixdim", pixdim, overwrite = TRUE)

  ## 2) Write the mask dataset (/mask)
  mask_arr <- as.array(mask)
  h5_write(file, "/mask", mask_arr@.Data, 
           dtype = hdf5r::h5types$H5T_NATIVE_UINT8, overwrite = TRUE)

  ## 3) Write the cluster map (/cluster_map)
  nVox <- sum(mask)
  h5_write(file, "/cluster_map", clusters@clusters, 
  ...
  h5_write(file, "/voxel_coords", vox_coords, 
           dtype = hdf5r::h5types$H5T_NATIVE_INT32, overwrite = TRUE)
  ...
  h5_write(clus_grp, "cluster_ids", cluster_ids, 
           dtype = hdf5r::h5types$H5T_NATIVE_INT32, overwrite = TRUE)
  ...
  if (!is.null(cluster_metadata)) {
    ...
    if (is.data.frame(cluster_metadata)) {
      for (coln in names(cluster_metadata)) {
        ...
        # Write using h5_write, need full path
        h5_write(file, file.path("/clusters/cluster_meta", coln), dat, 
                 dtype = hdf5r::guess_dtype(dat), overwrite = TRUE)
      }
    } else if (is.list(cluster_metadata)) {
      for (fld in names(cluster_metadata)) {
        dat <- cluster_metadata[[fld]]
        # Write using h5_write, need full path
        h5_write(file, file.path("/clusters/cluster_meta", fld), dat, 
                 dtype = hdf5r::guess_dtype(dat), overwrite = TRUE)
      }
    }
  }
  ...
  for (i in seq_along(vecs)) {
    ...
    for (fld in names(smeta)) {
      dat <- smeta[[fld]]
      # Write using h5_write, need full path
      h5_write(file, file.path("/scans", sname, "metadata", fld), dat, 
               dtype = hdf5r::guess_dtype(dat), overwrite = TRUE)
    }
    ...
    if (!summary_only) {
      ...
      for (cid in cluster_ids) {
        ...
        # Write using h5_write, need full path
        h5_write(file, file.path("/scans", sname, "clusters", dname), cdata,
                 dtype = hdf5r::guess_dtype(cdata),
      ...
      # Write using h5_write, need full path
      h5_write(file, file.path("/scans", sname, "clusters_summary", "summary_data"), summ_mat,
               dtype = hdf5r::guess_dtype(summ_mat),
               chunk_dims = chunk_dims,
               compression = compression,
               overwrite = TRUE)
    }
