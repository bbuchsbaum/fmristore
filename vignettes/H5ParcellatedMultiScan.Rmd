---
title: "Working with Clustered fMRI Data: H5ParcellatedMultiScan"
author: "fmristore Package"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Working with Clustered fMRI Data: H5ParcellatedMultiScan}
...
The `H5ParcellatedMultiScan` class provides an efficient way to store and access multi-run fMRI data that has been organized by spatial clusters (e.g., brain parcels or regions). This approach is particularly useful when:
...
1. **Full Runs** (`H5ParcellatedScan`): Store complete time series for every voxel
2. **Summary Runs** (`H5ParcellatedScanSummary`): Store only averaged time series per cluster

## Creating an H5ParcellatedMultiScan
...
## Reading and Using H5ParcellatedMultiScan

### Loading the Data

```{r load-experiment}
# Create H5ParcellatedMultiScan object
experiment <- H5ParcellatedMultiScan(h5_file)
...
# Find which run indices correspond to H5ParcellatedScan (full data)
full_run_indices <- which(sapply(experiment@runs, function(r) is(r, "H5ParcellatedScan")))
...
The H5ParcellatedMultiScan design provides several memory advantages:
