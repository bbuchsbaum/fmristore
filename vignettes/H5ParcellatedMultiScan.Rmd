...
cat("Created", length(runs_data), "runs with", 
...
legend("topright", legend = paste("Region", 1:3), 
       col = c("red", "green", "blue"), lty = 1)
```
...
write_dataset(list_of_neurovecs, file = "multi.h5", clusters = my_clusters,
              scan_names = c("run1", "run2", "run3"))

# Save only cluster averages (more compact)
write_dataset(my_neurovec, file = "summary.h5", clusters = my_clusters, 
              summary = TRUE)

# Use median instead of mean for summaries
write_dataset(my_neurovec, file = "median.h5", clusters = my_clusters,
...
```{r efficiency-demo}
# Compare file sizes
cat("Original in-memory data size per run: ~", 
    round(object.size(runs_data[[1]]) / 1024, 1), "KB\n")

cat("HDF5 compressed file size: ", 
    round(file.size(multi_file) / 1024, 1), "KB\n")

cat("Summary-only file size: ",
...
cat("Memory usage of small data extraction: ",
