...
### Note on memory warnings and lazy loading

`LabeledVolumeSet` stores volume data on disk and loads individual maps on demand. fmristore may warn when a call would materialize a very large dataset in R; for this vignette we temporarily disable those warnings via `options(fmristore.warn_memory = FALSE)` so the examples are not cluttered with performance messages.
