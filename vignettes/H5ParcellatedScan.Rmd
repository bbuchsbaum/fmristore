...
for (v in cluster_2_voxels) {
  x <- mask_indices[v, 1]; y <- mask_indices[v, 2]; z <- mask_indices[v, 3]
...
cat("Created", length(runs_data), "runs with timepoints:", 
...
write_dataset(runs_data[[3]], 
              ...
              summary = TRUE)  # Cluster averages only
...
matplot(task_voxel_ts[, 1:3], type = "l", lty = 1, lwd = 2,
        main = "Task Scan: Individual Voxel Time Series", 
        xlab = "Time Point", ylab = "Signal",
        col = c("red", "blue", "darkgreen"))
legend("topright", legend = paste("Voxel", voxel_indices[1:3]), 
...
matplot(cluster_2_ts[, 1:min(4, ncol(cluster_2_ts))], type = "l", lty = 1,
        main = "Task Activation in Cluster 2 (Motor) Voxels",
        xlab = "Time Point", ylab = "Signal",
        col = rainbow(min(4, ncol(cluster_2_ts))))
legend("topright", legend = paste("Voxel", cluster_2_voxels[1:min(4, ncol(cluster_2_ts))]),
...
plot(1:6, comparison_df$Ratio, type = "b", pch = 19, col = "darkgreen",
...
matplot(summary_matrix, type = "l", lty = 1, lwd = 2,
        ...
        col = c("red", "blue", "darkgreen"))
legend("topright", legend = paste("Cluster", 1:ncol(summary_matrix)),
       col = c("red", "blue", "darkgreen"), lty = 1, lwd = 2)
```
...
if (requireNamespace("corrplot", quietly = TRUE)) {
  corrplot::corrplot(cluster_correlations, method = "color", 
